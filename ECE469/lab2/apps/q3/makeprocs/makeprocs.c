...
void main (int argc, char *argv[])
{
  ...
  lock_t h_cb_lock;               // LOCK handle
  sem_t s_fullslots;
  sem_t s_emptyslots;
  char h_mem_str[10];             // Used as command-line argument to pass mem_handle to new processes
  char h_procs_sem_str[10];       // Used as command-line argument to pass page_mapped handle to new processes
  char h_cb_lock_str[10];         // Used as command-line arg
  char h_fullslots_str[10];
  char h_emptyslots_str[10];
  ...
  // Semaphores
  // Producer waits on s_emptyslots and signal s_fullslots
  if ((s_emptyslots = sem_create((BUFFER_SIZE-1))) == SYNC_FAIL) {
    Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  // Consumer waits on s_fullslots and signal s_emptyslots
  ...
  ditoa(s_emptyslots, h_emptyslots_str);
  ditoa(s_fullslots, h_fullslots_str);

  // Now we can create the processes.  Note that you MUST end your call to
  // process_create with a NULL argument so that the operating system
  // knows how many arguments you are sending.
  for(i=0; i < numprocs; i++) {
    process_create(CONSUMER_TO_RUN, h_mem_str, h_procs_sem_str, h_cb_lock_str, h_emptyslots_str, h_fullslots_str, NULL);
    process_create(PRODUCER_TO_RUN, h_mem_str, h_procs_sem_str, h_cb_lock_str, h_emptyslots_str, h_fullslots_str, NULL);
