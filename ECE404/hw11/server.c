...
/* Tested also with smaller value (5)
 * but it makes more sense to have 4096
 * because client also supports only 4096  */
...
int main(int argc, char *argv[])
{
  ...
  while(1) {
    ...
    while(1) {
       ...
       * so that we only copy the amount that we want.
       * which is the only to the size of str   */
      strncpy(str, recvBuff, MAX_DATA_SIZE);
      /* Put a null terminator at the end of str array
       * because strncpy does not automatically add a null terminator
       * if length of recvBuff > MAX_DATA_SIZE  */
      str[MAX_DATA_SIZE - 1] = '\0';
