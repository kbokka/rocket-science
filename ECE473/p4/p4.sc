...
(define (win~ b)
 ...
 (let ((win-0 (win b))
  ...
  (display approximate-win)(display b)(newline)
  (cond ((= win-0 1) 1)
 ...
 (define (maximize f l)
  (define (loop best-so-far l)
   (cond ((= best-so-far 1) 1)
	 ((null? l) best-so-far)
	 (else (loop (max (f (first l)) best-so-far) (rest l)))))
  (loop -1 l))
 ...
 (let ((win-0 (win b))
  ...
  (cond ((or (not (= win-0 0)) (null? m)) win-0)
	...
	(else (* (player b)
		 (maximize
		  (lambda (m)
		   (* (player b) (win~k (- k 1) (make-move m b)))) m))))))
...
(define (optimal-moves~ k b)
 (if (not (= (win b) 0)) (list)
     (let ((player-b (player b)))
	  ...
	  ;; finite depth
	  (remove-if-not
	   (lambda (m) (>= (* player-b (win~k (- k 1) (make-move m b)))
			   (* player-b (win~k k b)))) (moves b))))))
